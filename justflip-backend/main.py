...
from fastapi.security import OAuth2PasswordRequestForm #Это стандартная форма OAuth2 с полями username и password.
                                                       ...
                                                       #Чтобы указать, сколько времени будет действовать токен (например, 30 минут).
...
import secrets  #для генерации случайных паролей
from google.oauth2 import id_token  #для проверки токенов Google
from google.auth.transport import requests as google_requests
...
def create_app() -> FastAPI:
    ...
    @app.get("/")
    async def root():
        return {"message": "Welcome to JustFlip API! Visit /docs for documentation."}
